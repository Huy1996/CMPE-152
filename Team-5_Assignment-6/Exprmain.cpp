...
int main(int argc, const char *argv[])
{
    ...
    if (option == "-scan") {
        ...
        cout << ">>>>>> Open SemanticOut.txt for output. <<<<<<" << endl;
        freopen("SemanticOut.txt", "w", stdout);
        
        Semantics *check = new Semantics();
        check->visit(tree);

        int error_count = check->getErrorCount();
        if(error_count > 0){
            cout << endl << "There were " << error_count << " semantic errors." << endl;
        }

        SymtabStack *symtab_stack = check->get_symtab();
        CrossReferencer crossReferencer;
        crossReferencer.print(symtab_stack); 
    }
    else
